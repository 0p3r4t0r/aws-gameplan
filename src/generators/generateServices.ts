    ...
    fs.readdir(directoryPath, (err, directories) => {
        ...
        directories.forEach((directory) => {
            ...
            fs.readdir(path.join(directoryPath, directory), (err, files) => {
                ...
                const filteredFiles = files.filter(
                    (file) => !file.endsWith('_Dark_64.svg')
                )
        ...
        `)
                ...
                filteredFiles.forEach((file) => {
                    writeStream.write(
                        `import ${toCamelCase(file)}Data from '${path.join(
                            '../..',
                            directoryPath,
                            directory,
                            file
                ...
                filteredFiles.forEach((file) => {
                    ...
                    writeStream.write(
                        `${componentName}: () => <ServiceNode imgSrc={${componentName}Data} title="${componentName}" />,\n`
                    )
                })
                writeStream.write('\n')
