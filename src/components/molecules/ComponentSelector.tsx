...
const AwsServiceKey = ({ serviceKey, characterWidth }: { serviceKey: string, characterWidth: number }) => {
    ...
    return <span>
        +&nbsp;&nbsp;{serviceKey.substring(0, characterWidth)}<br/>
...
export const ComponentSelector = () => {
    ...
    const [serviceKeys, setServicesKeys] = useState(Object.keys(nodeTypes).sort());
    ...
    return (
        <div style={{
            ...
            <input style={{ margin: '10px 0', width: '100%' }} type="text" placeholder="🔎 Search" onChange={(e) => {
                ...
                if (searchString) {
                    setServicesKeys(sortedKeys.filter(key => normalize(key).includes(searchString)))
                } else {
            ...
            <div style={{
                ...
                {
                    serviceKeys.map((key) =>
                        <button style={{ textAlign: 'left', margin: '1px 0', padding: '0 4px', textWrap: 'balance' }} key={key} onClick={() => addNode(key)}>
